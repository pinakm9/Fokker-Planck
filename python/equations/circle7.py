...
num_layers = 3
nn_type = 'LSTM'
model_name = 'cricle7_{}_{}_{}'.format(num_nodes, num_layers, str(D).replace('.', '_'))
...
init_cond = lambda r: tf.reduce_mean((p(tf.zeros_like(r), r) - 0.5*r*r - v)**2)
@ut.timer
def diff_op(t, r):
    r2 = r*r
    z = 4.0*(r2 - 1.0)
...
grid_size=40
t, r = tf.split([[t, r] for t in np.linspace(time_[0]+5e-2, time_[1], num=grid_size) for r in np.random.normal(scale=3.0,\
                size=grid_size)], [1, 1], axis=1)
r_1 = tf.reshape(tf.linspace(tf.cast(0.0, p.dtype), R, num=1000), shape=(-1, 1))

p.solve(num_steps = 1000, num_samples = [grid_size**2, 1000], samples=[[t, r], [r_1]], sample_types = ['static', 'static'], initial_rate = 0.005)
