...
num_layers = 3
nn_type = 'LSTM'
model_name = 'cert2_{}_{}_{}'.format(num_nodes, num_layers, 'DGM')


p = dgm.DGM(dim = 2, num_nodes=num_nodes, num_layers = num_layers, final_activation=tf.keras.activations.linear)
...
def diff_op(t, r):
    ...
    a = (D/r + z*r) * p_r
    c = 4.0*(z + 2.0)
    eqn = - p_t + a - c + D * (p_rr - p_r**2)
    l1  = tf.reduce_mean(eqn**2)
    l2 = tf.reduce_mean((p(tf.zeros_like(r), r) - 0.5*r2 - log_2_pi)**2)
    return l1 + l2 if l2 < l1 else l2
