...
num_layers = 3
nn_type = 'LSTM'
model_name = 'cricle6_{}_{}_{}'.format(num_nodes, num_layers, str(D).replace('.', '_'))

p = dgm.DGM(dim = 2, num_nodes=num_nodes, num_layers = num_layers, final_activation=tf.exp, dtype=tf.float64)
...
class p_inf(tf.keras.models.Model):
    def __init__(self):
        super().__init__(dtype=p.dtype)
    def call(self, t, r):
        return (r**2 - 1.0)**2
...
p.solve(num_steps = 5000, num_samples = [grid_size**2, 1000], samples=[[t, r], [r_1]], sample_types = ['static', 'static'], initial_rate = 0.001)
...
plotter.animate(file_path='../../images/{}.png'.format(model_name), t=time_, x_lim=space_x, y_lim=space_y, z_lim=None) #wireframe=True)
#"""
plotter = nnp.NNPlotter(funcs=[p, p_1], space=[0.1*space_r], num_pts_per_dim=40)
plotter.animate(file_path='../../images/{}.mp4'.format(model_name), t=20.0*time_, x_lim=0.1*space_r, y_lim=None, z_lim=None)
