...
num_layers = 4
nn_type = 'LSTM'
model_name = 'cricle4_{}_{}_{}'.format(num_nodes, num_layers, str(D).replace('.', '_'))
...
init_cond = lambda r: 10.0*tf.reduce_mean((p(tf.zeros_like(r), r) - 0.5*r*r - v)**2)
@ut.timer
def diff_op(t, r):
    r2 = r*r
    z = 4.0*(r2 - 1.0)
...
#"""
"""
plotter = nnp.NNPlotter(funcs=[P], space=[2.0*space_x, 2.0*space_y], num_pts_per_dim=40)
plotter.animate(file_path='../../images/{}.mp4'.format(model_name + '_w2'), t=time_, x_lim=space_x, y_lim=space_y, z_lim=None, wireframe=True)
#"""
