...
def init_cond(p, r):
    return 100.0 * tf.reduce_mean((p(tf.zeros_like(r, dtype=dtype), r) - tf.exp(-0.5*r*r/sigma2 - log_2_pi_sigma))**2)

def integral_cond(p, t):
    return tf.reduce_mean((_2_pi * Gauss_quad(p, t) - tf.ones_like(t, dtype=dtype))**2)
